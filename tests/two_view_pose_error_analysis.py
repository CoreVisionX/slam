...
from registration.registration import FramePair, FramePairWithGroundTruth  # noqa: E402
...
BASE_SEED = 0
MAX_DEPTH = 20.0
SAMPLING_MODE = "index_offset"
MIN_INDEX_OFFSET = 9
MAX_INDEX_OFFSET = 9
RESULTS_DIR = Path(__file__).parent / "results"
RESULTS_PATH = RESULTS_DIR / "relative_pose_errors.npz"
...
# %%
def estimate_relative_pose(pair_with_gt: FramePairWithGroundTruth) -> dict[str, Any]:
...
# %%
# Sample pairs, estimate poses, and collect error metrics
error_records: list[dict[str, Any]] = []
failed_samples: list[tuple[int, str]] = []

for idx in tqdm(range(NUM_SAMPLES), desc="Sampling pairwise pose estimates"):
    seed = BASE_SEED + idx
    pair = test_utils.load_tartanair_pair(
        env=ENVIRONMENT,
        traj=TRAJECTORY,
        difficulty=DIFFICULTY,
        max_dist=MAX_DISTANCE,
        max_degs=MAX_DEGREES,
        seed=seed,
        sampling_mode=SAMPLING_MODE,
        min_index_offset=MIN_INDEX_OFFSET,
        max_index_offset=MAX_INDEX_OFFSET,
    )
    ...
    ground_truth_pose = pair.first_T_second
    pose_error = ground_truth_pose.between(result["estimated_pose"])

    translation_error = np.asarray(pose_error.translation())
    rotation_error = gtsam.Rot3.Logmap(pose_error.rotation())
    estimated_translation = result["estimated_pose"].translation()
    estimated_rotation = result["estimated_pose"].rotation()

    first_index = int(getattr(pair, "first_index", -1))
    second_index = int(getattr(pair, "second_index", -1))
    first_frame_id = int(getattr(pair, "first_frame_id", first_index))
    second_frame_id = int(getattr(pair, "second_frame_id", second_index))
    if first_frame_id >= 0 and second_frame_id >= 0:
        mid_frame_id = float(first_frame_id + second_frame_id) / 2.0
        frame_id_gap = float(second_frame_id - first_frame_id)
    else:
        mid_frame_id = float("nan")
        frame_id_gap = float("nan")
    ...
    error_records.append(
        {
            ...
            "inliers": int(len(result["matched_pair"].matches)),
            ...
            "second_frame_id": second_frame_id,
            "mid_frame_id": mid_frame_id,
            "frame_id_gap": frame_id_gap,
        }
    )
...
error_vectors = np.stack(
    [np.concatenate([record["translation_error"], record["rotation_error"]]) for record in error_records],
    axis=0,
)
...
print(f"Saved error data for {len(error_records)} samples to {RESULTS_PATH}")
if failed_samples:
...
estimated_translation_threshold = 5.0  # e.g., set to 15.0 to drop very long motions
estimated_rotation_threshold_deg = None  # e.g., set to 30.0 to drop wide rotations
inlier_min = 150
...
plt.colorbar(scatter_est_trans, ax=axes[1], label="Inlier Count")
plt.colorbar(scatter_est_rot, ax=axes[2], label="Inlier Count")
...
if error_vectors_masked.shape[0] < 6:
    ...
    # Use coarse binning to keep the number of clusters manageable
    ...
    # Keep only the most populated clusters to avoid clutter
...
combinations = [
    ...
    ("inliers", "est_trans_norm", "est_rot_norm_deg"),
    ("inliers", "est_trans_norm", "est_rot_norm_deg", "gt_trans_norm", "gt_rot_norm_deg"),
]
