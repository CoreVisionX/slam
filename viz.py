...
def rr_log_pose(path: str, pose: gtsam.Pose3, frame: RectifiedStereoFrame | StereoDepthFrame, camera_xyz: rr.ViewCoordinates = rr.ViewCoordinates.RDF, image_plane_dist: float = 0.2, compress: bool = True):
...
def rr_log_pose_arrows(path: str, pose: gtsam.Pose3, arrow_length: float = 0.2):
    rr.log(path + "/pose", rr.Arrows3D(vectors=[[arrow_length, 0.0, 0.0], [0.0, arrow_length, 0.0], [0.0, 0.0, arrow_length]], colors=[[255, 0, 0], [0, 255, 0], [0, 0, 255]], origins=[[0, 0, 0], [0, 0, 0], [0, 0, 0]]))
